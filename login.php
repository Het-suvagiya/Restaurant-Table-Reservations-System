...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($email) || empty($password)) {
        ...
        $query = "
            SELECT u_id as id, u_password as password, " . ROLE_USER . " as role FROM tbl_users WHERE u_email = ?
            UNION
            SELECT m_id as id, m_password as password, " . ROLE_MANAGER . " as role FROM tbl_manager WHERE m_email = ?
            UNION
            SELECT a_id as id, a_password as password, " . ROLE_ADMIN . " as role FROM tbl_admin WHERE a_email = ?
        ";
        ...
        if ($user) {
            if (password_verify($password, $user['password'])) {
                ...
                // if manager account, verify has at least one approved restaurant
                $finalRole = $user['role'];
                if ($finalRole == ROLE_MANAGER) {
                    $chk = $conn->prepare("SELECT COUNT(*) as cnt FROM restaurants WHERE manager_id = ? AND status = 'approved'");
                    $chk->bind_param("i", $user['id']);
                    $chk->execute();
                    $cnt = $chk->get_result()->fetch_assoc()['cnt'];
                    $chk->close();
                    if ($cnt == 0) {
                        // manager exists but no approved restaurant yet -> treat as normal user
                        $finalRole = ROLE_USER;
                    }
                }
...
<div class="min-h-screen flex items-center justify-center p-5 pt-32 pb-24 animate-soft-rise relative overflow-hidden">
    ...
    <div class="bg-white rounded-[40px] shadow-premium border border-black/5 w-full max-w-[480px] p-8 md:p-12 relative">
        ...
        <div class="mb-10 mt-4 text-center">
            <h1 class="font-zodiak text-4xl text-ink font-bold mb-3">Welcome Back</h1>
            <p class="text-ink/60 text-sm">Sign in to book your next favorite table.</p>
        </div>
        ...
        <form method="POST" class="flex flex-col gap-6">
            <div>
                <label class="block text-[10px] font-bold uppercase tracking-widest text-ink/50 mb-2">Email
                    Address</label>
                <input type="email" name="email" required placeholder="example@email.com"
                    class="w-full bg-slate-50 border border-black/5 rounded-2xl px-5 py-4 text-sm text-ink font-medium focus:outline-none focus:ring-2 focus:ring-deepTeal/20 focus:border-deepTeal transition-all placeholder:text-ink/20">
            </div>
            <div>
            ...
            <button type="submit"
                ...
                Sign In <i class="fas fa-arrow-right text-sm transition-transform group-hover:translate-x-1"></i>
            </button>
        </form>
        ...
        <div class="mt-8 text-center border-t border-black/5 pt-8">
            <p class="text-sm text-ink/60 font-medium">
                ...
                <a href="register.php"
                    class="text-deepTeal hover:text-tealAccent font-bold ml-1 transition-colors underline decoration-2 decoration-deepTeal/20 underline-offset-4">Register</a>
            </p>
        </div>
    </div>
</div>
